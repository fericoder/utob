@extends('.dashboard.layouts.master',  ['title' => 'لیست بیمارستان ها'])
                    ...
                    <table class="m-datatable" id="html_table" width="100%">
                        ...
                        @foreach ($hospitals as $hospital)
                            <tr>
                                <td>{{ $hospital->id  }}</td>
                                <td>{{ $hospital->name  }}</td>
                                <td>{{ jdate($hospital->created_at)  }}</td>
                                {{--<td>{{ jdate($hospital->published_at)  }}</td>--}}
                                {{--<td>{{ $hospital->viewCount  }}</td>--}}
                                {{--<td>{{ $hospital->commnetCount  }}</td>--}}
                                <td>
                                    <a href=""
                                       data-id="{{$hospital->id}}" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill button"
        ...
        $(document).on('click', '.button', function (e) {
            ...
            swal({
                ...
                function() {
                    $.ajax({
                        type: "post",
                        url: "{{url('dashboard/hospital/delete')}}",
                        data: {id:id},
